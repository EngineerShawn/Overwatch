...
module.exports = {
    ...
    run: async (client, interaction, args) => {
        ...
        if (args.inspect) {
            const embed = new client.Embed().setTitle(
                `${characterName}'s Money`,
            );
            let desc = "";
        ...
        if (args.amount < 0)
        {return interaction.reply({
            content: "Please only use positive amounts",
            ephemeral: true,
        });}
        ...
        if (args.add) {
            if (
                ...
                client.utils.log.info(
                    `[${interaction.member.displayName}] Added ${
                        args.amount
                    } to ${GetPlayerName(args.id)} (${args.id})'s ${
                        args.moneytype
                    } [Previously: ${prevMoney}]`,
                );
                return interaction.reply({
                    content: `${characterName} (${args.id})'s ${
                        args.moneytype
                    } has increased from ${prevMoney} to ${player.Functions.GetMoney(
                        args.moneytype,
                    )}`,
                    ephemeral: false,
                });
                ...
                return interaction.reply({
                    content:
                        "Something went wrong trying to add money to this player",
                    ephemeral: false,
                });
            }
        } else if (args.remove) {
            if (
                ...
                client.utils.log.info(
                    `[${interaction.member.displayName}] Removed ${
                        args.amount
                    } from ${GetPlayerName(args.id)} (${args.id})'s ${
                        args.moneytype
                    } [Previously: ${prevMoney}]`,
                );
                return interaction.reply({
                    content: `${characterName} (${args.id})'s ${
                        args.moneytype
                    } has decreased from ${prevMoney} to ${player.Functions.GetMoney(
                        args.moneytype,
                    )}`,
                    ephemeral: false,
                });
                ...
                return interaction.reply({
                    content:
                        "Something went wrong trying to remove money from this player",
                    ephemeral: false,
                });
            }
        } else if (args.set) {
            if (
                player.Functions.SetMoney(args.moneytype, args.amount, reason)
            ) {
                client.utils.log.info(
                    `[${interaction.member.displayName}] Set ${GetPlayerName(
                        args.id,
                    )} (${args.id})'s ${args.moneytype} to ${
                        args.amount
                    } [Previously: ${prevMoney}]`,
                );
                return interaction.reply({
                    content: `${characterName} (${args.id})'s ${
                        args.moneytype
                    } has been set to ${player.Functions.GetMoney(
                        args.moneytype,
                    )} (Previously: ${prevMoney})`,
                    ephemeral: false,
                });
                ...
                return interaction.reply({
                    content:
                        "Something went wrong trying to set this player's money",
                    ephemeral: false,
                });
            }
        }

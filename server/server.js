...
SetConvarReplicated("overwatch_servername", z.config.FiveMServerName);
SetConvarReplicated("overwatch_discordinvite", z.config.DiscordInviteLink);
SetConvarReplicated("overwatch_serverip", z.config.FiveMServerIP);
SetConvarReplicated("overwatch_userpresence", String(z.config.enableUserPresence));
...
on("playerJoining", (oldId) => {
    ...
    if (z.bot.isRolePresent(member, z.config.StaffChatRoleIds)) {
        ExecuteCommand(`add_principal "player.${source}" group.overwatchstaff`);
    }
});
...
if (z.config.EnableStaffChatForwarding) {
    RegisterCommand("staff", (source, args, raw) => {
        if (!IsPlayerAceAllowed(source, "overwatch.staffchat")) return;
    ...
    RegisterCommand("stafftoggle", (source, args, raw) => {
        if (IsPlayerAceAllowed(source, "overwatch.staffchat")) {
            ExecuteCommand(`remove_principal "player.${source}" group.overwatchstaff`);
